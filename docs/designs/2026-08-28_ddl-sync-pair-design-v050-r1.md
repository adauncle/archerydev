...
### 9.1 Phase 1 · 短期 C · **5 个工作日** (8/31 周一 - 9/4 周五) — 8/28 用户拍板

**核心: 库对管理 + 批量导入 + 一键配 + 走当前配置流程 + 推 110 prod**

8/28 17:40 DBA 阿达叔叔拍板: "**按照每周 5 天工作日规划**" — 5 天紧凑节奏, 不是 3 天 (周一设计+开发 / 周二开发 / 周三提测+发布), 也不是 2-3 天 (原设计稿 §9.1 拍板). 每周 5 天工作日是 DBA 真实节奏, 留足时间修复实战踩坑.

**5 天日程表**:

| 工作日 | 主要工作 | 详细说明 |
|--------|----------|----------|
| **8/31 周一** | 详细设计 + 数据模型 migration | 数据模型 3 张表 migration (sync_mode 默认 blacklist + r2 sync_type 字段) + 库对管理列表 + 库对详情设计稿, 含 r1+r2+r3 决策落地 |
| **9/1 周二** | 库对管理 + r1 批量导入 开发 | 库对管理 CRUD (DdlSyncPair + DdlSyncTable) + 5 个核心按钮 (🎯 一键配 / 📥 批量导入 / + 添加同步表 / 🔍 schema 差集 / ⚙ 过滤规则) + r1 批量导入 (从历史库扫表 + 模态框 + 过滤规则) |
| **9/2 周三** | r2 一键配 + 134 dev 端到端演练 | r2 一键配机制 (compute_diff + one_click_setup + DdlSyncTable.sync_type 字段) + 134 dev 端到端演练: accesscard 库对 + 一键配 1-click 接受 + 1 条真实 DDL 联动, 验证 r3 走当前配置流程 |
| **9/3 周四** | 提测 + 修复实战踩坑 | DBA 验收用例 + 业务 RD 端到端测试 + 修复实战踩坑 (避坑 8/26 推 110 实战踩坑: CACHE_URL / SECRET_KEY / K3 变量 / ALLOWED_HOSTS / poller zombie / rollback import) |
| **9/4 周五** | 推 110 prod + smoke test + 收尾 | 上午按 5 步必做推 110 prod (跟 v0.4.5 + v0.3.0 联动), 下午 smoke test (5 端点全过 + 业务 RD 浏览器实测) + 文档收尾 (changelog + 推 110 实战踩坑总结) + 9/11 周报准备 |
