...
| 异常保护 | 无 | **rollback 端点 + 钉钉通知 + 异常回滚** (联动 智能回滚 + 钉钉 OA) |
...
8. [联动点](#8-联动点)
9. [风险与验证](#9-风险与验证)
10. [R2 进一步优化: 一键配 (按历史库)](#10-r2-进一步优化-一键配-按历史库)
11. [R3 重大决策变更: 走当前配置的流程](#11-r3-重大决策变更-走当前配置的流程)
...
| 已有功能 | 联动方式 | 业务价值 |
|---|---|---|
| DDL 智能回滚 (R 之前已落地, commit `e54a663`) | 历史库 DDL 工单也走智能回滚 | 历史库 DDL 失败能自动回 |
| 无锁变更 gh-ost (R 之前已落地, commit `cd2ce88`) | 历史库 DDL 大表走无锁变更 | 历史库大表 DDL 也无锁 |
| 大表 DDL 防呆 (R 之前已落地, commit `374d990`) | 历史库 DDL 也走大表防呆 | 历史库大表 DDL 不锁表 |
| 钉钉 OA (R 之前已落地, commit `d5f88d1`) | 历史库 DDL 审批人推钉钉 | DBA 钉钉里就能审 |
...
### 10.4 数据模型微调 (DdlSyncTable 加 sync_type 字段)
...
1. **业务库 DDL 必审过 trigger** — 见 §11.3 第 2 条
2. **历史库执行前预演 + 异常 rollback** — 跟 智能回滚 联动 (R 之前已落地). 复用 R 之前拍的 rollback 端点 (`services.db._get_creds` + DROP TABLE IF EXISTS 兜底)
3. **异常通知 DBA + 业务 RD** — 跟 钉钉 OA 联动, 异常立刻钉钉通知 DBA + 业务 RD, 不依赖业务 RD 自己查工单状态

### 11.5 Archery 配置 (DBA 后台自己配, R1 不动)
...
- **R2 一键配** (§10): 库对配白/黑名单, 跟"自动生成的工单走哪个流程" 无关, 保留
- **R3 重大决策** (§11, 本次新增): 明确"自动生成的镜像工单走当前 Archery 配置的流程", 跟正常工单一样 + 3 个安全护栏
...
- **** (本次) R2 加 §10 一键配机制, 业务库 1589 / 历史库 1289 实战数据支撑
