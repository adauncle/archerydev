...
# -*- coding: utf-8 -*-
"""
D11 hotfix: 推 archery/settings.py 修 namespace bug + kill gunicorn + verify
"""
import io
import sys
import paramiko
...
REMOTE_CMDS = [
    # 1. 备份
    'cp /opt/archery/prod/archery/settings.py /opt/archery/prod/archery/settings.py.bak_20260902_1055',
    # 2. chown
    'chown -R archery:archery /opt/archery/prod/archery/',
    # 3. 清 __pycache__
...
def main():
    ...
    try:
        # 1. SFTP 推
        sftp = ssh.open_sftp()
        local_path = LOCAL_BASE + chr(92) + 'archery' + chr(92) + 'settings.py'
        remote_path = f'{REMOTE_BASE}/archery/settings.py'
        print(f'PUT settings.py -> {remote_path}')
