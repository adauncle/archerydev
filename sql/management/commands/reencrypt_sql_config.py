...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 拿 cipher
        key = getattr(settings, "MIRAGE_SECRET_KEY", None) or settings.SECRET_KEY[:32]
        cipher = Crypto(key)
        self.stdout.write(f"使用 mirage key (前 32 字符): {key[:8]}...{key[-4:]} (len={len(key)})")
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING("DRY-RUN 模式：只打印前 5 条，不 UPDATE"))
            for row_id, item, plain in rows[:5]:
                self.stdout.write(f"  id={row_id} item={item!r} value={plain!r}")
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(f"完成：加密 {updated} 条，跳过空值 {skipped} 条，总 {len(rows)} 条")
        )
